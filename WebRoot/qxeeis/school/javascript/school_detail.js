...
var data_table = function() {
	...
	$.ajax({
		url: "/xyeeis/servlet/QxSchooldetailServlet",
		type: "post",
		data: {
	...
	$(".downLoad").click(function(){
		...
		location.href = "/xyeeis/servlet/QxExportExcelServlet?schoolName="+schoolName+"&currentdate="+date;
	});
	
	
/*	$.getJSON("../json/basicInfo_school_area_xiangyang_"+id+".json",function(data) {
...
var fPart = function() {
	...
	//传输请求
	$.ajax({
		url: "/xyeeis/servlet/QxSchoolInforServlet",
		type: "post",
		data: {
			"schoolName": schoolName,
			"currentdate":date
		...
		success: function(json) {
			var data = JSON.parse(json);
			buildNettable("#left_side",""+data[0].name+"、"+data[1].name+"和襄阳市"+date+"月份的平均水平对比情况",temp_index,data);
		},
		error: function() {
			alert("失败");
		}
