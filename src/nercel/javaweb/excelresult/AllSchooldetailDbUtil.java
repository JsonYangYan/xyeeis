...
public class AllSchooldetailDbUtil {
	 ...
	 * @throws ClassNotFoundException
	 */
	public ArrayList getschoolName(String schoolArean,String time) throws SQLException,
			ClassNotFoundException {
	 ...
	 * @throws ClassNotFoundException
	 */
	public ArrayList<Integer> getSchoolId(String schoolArean,String time)
		...
		while (rs.next()) {
			listSchoolId.add(rs.getInt("autoId"));
		}
		state.close();
		return listSchoolId;
	}
	...
	public ArrayList getDetail(String schoolArean,String time) throws SQLException,
		...
		for (int i = 0; i < listSchoolId.size(); i++) {
			...
			mapSchool = getSchooldetail(Integer.parseInt(listSchoolId.get(i).toString()),time); // 获取一个学校的填报信息
			list.add(mapSchool);
		}
		return list;
	}
